...
def shouldUpdateScreen(images):
    for image in images.values():
        matches = positions(image)
        if len(matches) > 0:
            return False
...
def main():
    ...
    while True:
        ...
        if shouldUpdateScreen(images) and now - last['update_screen'] > addRandomness(update_screen * 60):
            sys.stdout.flush()
