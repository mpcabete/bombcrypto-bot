...
def checkConnection():
    ...
    if okBtnScreen:
        ...
        if telegram_notify > c['telegram']['error_count'] and telegram_notify is True:
            bot.send_message(c['telegram']['chat_id'],"Tela do Cryptobomb não encontrada. Verificar navegador.",disable_notification=True)
            telegram_notify = 0
        time.sleep(60)
        return False
    else:
